...
fn topogicalSort(adjacencyMatrix: []const []const i32, nodeList: std.ArrayList(i32)) !std.ArrayList(i32) {
    std.debug.print("top sort\n", .{});
    var visited = std.ArrayList(i32).init(allocator);
    var previousLength: usize = 0;
    for (nodeList.items) |node| {
        if (contains(visited, node) == null) {
            try dfs(adjacencyMatrix, node, nodeList, &visited);
            printArray(visited.items[previousLength..visited.items.len]);
            for (0..visited.items.len - previousLength) |insertionIndex| {
                try visited.insert(insertionIndex, visited.pop());
            }
            previousLength = visited.items.len;
        }
    }
    return visited;
}

fn swapUntilItWorks(adjacencyMatrix: []const []const i32, update: *std.ArrayList(i32)) !void {
    for (0..update.items.len) |_| {
        for (0..update.items.len - 1) |index| {
            var updateClone = try update.clone();
            for (0..index) |_| {
                _ = updateClone.orderedRemove(0);
            }
            const first = updateClone.orderedRemove(0);
            if (!(try areNodesReachable(adjacencyMatrix, first, update.*, &updateClone))) {
                const temp = update.items[index];
                update.items[index] = update.items[index + 1];
                update.items[index + 1] = temp;
            }
            updateClone.deinit();
        }
    }
...
pub fn main() !void {
    ...
    for (updates.items) |update| {
        ...
        if (!(try isUpdateCorrect(adjacencyMatrix, &clonedUpdate))) {
            //const newUpdate = try topogicalSort(adjacencyMatrix, update);
            var updateClone = try update.clone();
            try swapUntilItWorks(adjacencyMatrix, &updateClone);
            printArray(updateClone.items);
            result += updateClone.items[updateClone.items.len / 2];
            updateClone.deinit();
        }
    }
